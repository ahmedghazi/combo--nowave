...
const Header = ({ settings }: Props) => {
  ...
  useEffect(() => {
    ...
    if (!hasInitialized.current || logosLottie) {
      if (logosLottie && logosLottie.length > 0) {
        const randomIndex = Math.floor(Math.random() * logosLottie.length);
        console.log("randomIndex", randomIndex);
        if (logosLottie[randomIndex] && logosLottie[randomIndex].asset) {
          // Use the async urlForFile function
          urlForFile(logosLottie[randomIndex].asset).then((url) => {
            if (url) {
